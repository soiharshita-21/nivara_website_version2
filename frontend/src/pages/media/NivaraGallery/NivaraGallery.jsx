...
const NivaraGallery = () => {
  ...
  galleryItems.forEach((item) => {
    ...
    // Cover image is the latest image based on ID
    if (!foldersMap[catName].latestImage || item.id > foldersMap[catName].latestImageId) {
      foldersMap[catName].latestImage = item.image_url;
      foldersMap[catName].latestImageId = item.id;
    }
  });
  ...
  const folders = Object.values(foldersMap).sort((a, b) => {
    const maxIdA = Math.max(...a.images.map(img => img.id));
    const maxIdB = Math.max(...b.images.map(img => img.id));
    ...
    <div className="gallery-page">
      ...
      {loading ? (
        <div className="loading-container" style={{ textAlign: "center", padding: "80px 20px" }}>
          <p style={{ fontSize: "1.2rem", color: "#64748b" }}>Loading Nivara Gallery...</p>
        </div>
      ) : selectedFolder ? (
        ...
        <div className="folder-grid">
            ...
            return (
              <div className="folder-card" key={folder.title} onClick={() => setSelectedFolder(folder.title)}>
                <div className="folder-icon">
                  ...
                  <img className="folder-preview-img" src={folder.latestImage} alt={folder.title} />
          ...
          {folders.length === 0 && (
            <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "80px 20px" }}>
              <p style={{ color: "#64748b", fontSize: "1.1rem" }}>No folders found in the gallery.</p>
            </div>
          )}
        </div>
      )}
