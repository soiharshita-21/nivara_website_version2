...
const GetAppointment = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSubmitting(true);
    setAlertState({ type: "info", message: "Scheduling appointment..." });

    try {
      const response = await fetch("http://localhost:5001/api/appointments/apply", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setAlertState({ type: "error", message: data.message || "Failed to schedule appointment." });
      }
    } catch (err) {
  ...
  return (
    <div className="loan-form-page">
      <form className="loan-form" onSubmit={handleSubmit}>
        <h2 className="form-title" style={{ fontSize: '28px', fontWeight: 600, textAlign: 'center', marginBottom: '35px' }}>
