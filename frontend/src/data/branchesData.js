...
export const defaultBranchesByState = {
  KARNATAKA: [
    {
      city: "JP Nagar",
      opened: "2015-05-11",
...
export const defaultBranchesData = Object.keys(defaultBranchesByState).reduce((acc, state) => {
...
export const defaultAllBranches = Object.values(defaultBranchesData).flat();

export const getTotalBranchesCount = (branchesByState = defaultBranchesByState) => {
  if (!branchesByState || typeof branchesByState !== "object") return 0;
  return Object.values(branchesByState).reduce((total, list) => {
...
export const fetchBranchesFromApi = async () => {
  ...
  try {
    ...
    console.warn("Backend branches API unavailable, using fallback branches data:", err?.message || err);
  }
  return null;
};
